...
router.get('/attendance/:studentId', guard, (req, res) => {
  ...
  const rows = db.prepare(`
    SELECT a.status, a.marked_at, ls.date, ls.topic, g.name AS group_name
    FROM attendance a
    JOIN lesson_sessions ls ON ls.id = a.lesson_session_id
    LEFT JOIN groups g ON g.id = ls.group_id
    WHERE a.student_id = ? ORDER BY ls.date DESC LIMIT 100
  `).all(sid);
  res.json({
    visitsLeft: crm ? crm.visits_left : null,
    subscriptionStatus: crm ? crm.status : null,
    records: rows.map(r => ({ status: r.status, date: r.date, topic: r.topic || '', groupName: r.group_name || '', markedAt: r.marked_at })),
  });
});

// Отзывы (только не внутренние)
